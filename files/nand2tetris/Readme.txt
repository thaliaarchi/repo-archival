...
------------------------------------------------
The Nand2Tetris Software Suite allows the implementation in Java of new chips
for
use with the Hardware Simulator via the Chip API which is henceforth
described.
...
implementations of chips provided with the Nand2Tetris Software Suite. The
HDL
...
Tutorial supplied at www.nand2tetris.org) and specify the chip name and all
the input and
output pins in the usual manner. Instead of the list of gates which usually
comprise the implementation, the line:
BUILTIN ChipName;
should appear (where ChipName should be substituted for the name of the
chip).
In addition to the HDL file for the chip, each chip should be implemented in
a
...
the compiled counterparts of the aforementioned chip provided with the
Nand2Tetris
...
The code for the chip may access the input and outpu pins via the data
members
inputPins and outputPins, respectively.
The value of a b-bit input pin/bus which was declared n-th (starting from
zero)
in the chip HDL file can be accessed by evaluating the b least significant
bits
of the value returned by calling inputPins[n].get().
The value of a b-bit output pin/bus which was declared n-th (starting from
zero)
in the chip HDL file can be updated to v (a value in which all but the v
least
significant bits are zero) by calling outputPins[n].set(v).
The Nand2Tetris Software Suite Hardware Simulator Chip Java API also provides
support
for implementing a GUI visualization of the chip (similar to the one
implemented by the provided ALU, RAM*, ROM32K, ARegister and DRegister
chips).
...
-----------------------------------------------------------------
The Nand2Tetris Software Suite allows the implementation in Java of new VM
functions
...
include e.g. time & date queries, random number generation using an RNG
daemon
...
function, if such an implementation exists. This priority mechanism is
similar
...
through the implementation of the Jack OS supplied in the Nand2Tetris
Software Suite.
...
constructors or methods in this API but only a concept of functions.
Functions
may conceptually be class methods and therefore receive a this-style pointer
as the first argument (e.g., as the String.* functions do) or may
conceptually
...
VM data is 16-bit quantities and Java shorts are 16-bit quantities, all of
the
...
As with normal .vm files (see the VM Emulator Tutorial at
www.nand2tetris.org), all
...
implementing the Jack OS supplied in the Nand2Tetris Software Suite are
located
under the InstallDir/builtInVMCode directory).
A Java static method which implements a VM function may communiate with the
Virual Machine using any of the following static methods which it inherits
from
...
argument is an int and not a short for
 ...
 ranges HEAP_START_ADDRESS - HEAP_END_ADDRESS
or
...
may or may not be implemented in Java)
is
called.
short callFunction(String functionName,
...
implementing a VM function SHOULD NOT
enter
